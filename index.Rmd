...
The Graphics Companion provides the code for different data visualisations that can be created using the [ggplot2](https://cran.r-project.org/web/packages/ggplot2/index.html) [R](https://cran.r-project.org/) package.

The Graphics Companion adopts the structure of the [Financial Times' Visual Vocabulary](https://ft.com/vocabulary) by categorising different chart types by the data relationships that they best illustrate.

The data used throughout the Graphics Companion derive from Hans Rosling's [Gapminder World](https://www.gapminder.org/) which available in the R [gapminder](https://cran.r-project.org/web/packages/gapminder/index.html) package. Data on life expectancy at birth, GDP per capita and total population are provided for 142 countries between 1952 and 2007.

## Setup

You need to install (once only) the [tidyverse](https://www.tidyverse.org/) package and load it into your R session. [ggplot2](https://cran.r-project.org/web/packages/ggplot2/index.html) is part of the tidyverse suite of R tools for data science.
```{r}
...
ggplot(df, aes(x = continent, y = median)) +
  geom_col(fill = "#fc6721", alpha = 0.8) +
  scale_y_continuous(expand = c(0, 0), labels = scales::dollar) +
  labs(title = "",
